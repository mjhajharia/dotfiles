...
print(args.transform, args.datakey)

stan_filename=f'stan_models/{args.transform}_DirichletSymmetric.stan'

with open(stan_filename, 'w') as f:
    f.write(f'#include target_densities/DirichletSymmetric.stan{os.linesep}#include transforms/{args.transform}.stan{os.linesep}')
    f.close()
...
try:
    ...
    bsmodel = bs.StanModel.from_stan_file(stan_filename, data, make_args=["STAN_THREADS=True"], stanc_args=[f"--include-paths='/mnt/home/mjhajaria/simplex-transforms/'"])
