...
We have prepared a **[quarto template](template6.html)** to help you get started. 
...
::: {.tmpl}
This is the template for [assignment 6](assignment6.html). You can download the [broken stan-file](https://raw.githubusercontent.com/avehtari/BDA_course_Aalto/gh-pages/assignments/additional_files/assignment6_linear_model.stan) and the [qmd-file](https://raw.githubusercontent.com/avehtari/BDA_course_Aalto/gh-pages/assignments/template6.qmd) or copy the code from this rendered document after clicking on `</> Code` in the top right corner.
...
You can download the [broken stan file from github](https://raw.githubusercontent.com/avehtari/BDA_course_Aalto/gh-pages/assignments/additional_files/assignment6_linear_model.stan).
...
:::
::: {.answer}
```{.stan include="assignment6_linear_model.stan"}
```
:::
...
* Has the sampling success been verified/summarized (e.g. by inspecting and summarizing the output of [`CmdStan`'s `diagnose` method](https://mc-stan.org/docs/cmdstan-guide/diagnose.html), see also above).
    * No.
    * Yes, but partially.
    * Yes, and it is correctly verified.
* Fix #1: Is there a fix for line 10[`original`: `real<upper=0> sigma;`, `possible fix: real<lower=0> sigma;`]{.answer}.
...
* Fix #2: Is there a fix for line 16 [`original`: `y ~ normal(mu, sigma)`, `possible fix: `y ~ normal(mu, sigma);`]{.answer}.
    * It has not been discussed, that this line should be fixed.
    * It has been discussed, that this line should be fixed, but there is no fix presented for it or the fix is clearly wrong.
    * There is a fix presented for this line, that clearly solves the problem.
* Fix #3: Is there a fix for line 19 [`original`: `real ypred = normal_rng(mu, sigma);`, `possible fix`: `real ypred = normal_rng(alpha + beta*xpred, sigma);`]{.answer}.
    * It has not been discussed, that this line should be fixed.
    * It has been discussed, that this line should be fixed, but there is no fix presented for it or the fix is clearly wrong.
    * There is a fix presented for this line, that clearly solves the problem.
* Have the red lines been correctly described ([5, 50, and 95 percentiles of the predictions]{.answer})?
    * No
    * Yes
* Have the grey lines been correctly described ([5, 50, and 95 percentiles of the mean]{.answer})?
    * No
    * Yes
* Has the difference between the red and grey lines been explained ([The predictions have a higher spread due to the observation model (or something like that).]{.answer})?
    * No
    * Somewhat
    * Yes
* Has the student retention trend been described ([It's going down.]{.answer})
    * No
    * Yes
* Has the predictive performance for the held out data been discussed and assessed satisfactorily ([It's not super bad, but it also isn't great]{.answer})?
    * No
    * Somewhat
    * Yes
* Has at least one way to improve the model been mentioned [E.g. Informative Priors]{.answer}?
:::
...
like in the Figure below [ADD FIGURE]{.answer}?
    * No plot included
