...
```{julia}
scatter(unc_sample_array[:, 1], unc_sample_array[:, end], alpha=0.1, label="Centered")
```


```{julia}
scatter(unc_sample_array_nc[:, 1], unc_sample_array_nc[:, end], alpha=0.1, label="Non-Centered")
```
