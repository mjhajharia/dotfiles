...
```{julia}
scatter(unc_sample_array[:, 1], unc_sample_array[:, end], alpha=0.1, label="Centered", fmt=:png, xlabel="theta[1]", ylabel="tau")
```


```{julia}
scatter(unc_sample_array_nc[:, 1], unc_sample_array_nc[:, end], alpha=0.1, label="Non-Centered", fmt=:png, xlabel="theta[1]", ylabel="tau")
```
...
```{julia}
scatter(unc_sample_array[:, 1], unc_sample_array[:, end], alpha=0.1, label="log(Condition Number of Hessian)", 
zcolor=log.(hessian_df_cholesky[!, "CN_Hessian"]), title="Centered Model", fmt=:png)
```


```{julia}
scatter(unc_sample_array_nc[:, 1], unc_sample_array_nc[:, end], alpha=0.1, label="log(Condition Number of Hessian)", 
zcolor=log.(hessian_df_cholesky_nc[!, "CN_Hessian"]), title="Non-Centered Model", fmt=:png)
```
...
```{julia}
scatter(hessian_df_cholesky[:, :CN_Hessian], hessian_df_cholesky[:, :CN_Transformed_Hessian], xaxis=:log, yaxis=:log, alpha=0.1, label="Centered", font="Palatino", fmt=:png)
scatter!(hessian_df_cholesky_nc[:, :CN_Hessian], hessian_df_cholesky_nc[:, :CN_Transformed_Hessian], xaxis=:log, yaxis=:log, alpha=0.1, label="Non Centered", fmt=:png)
