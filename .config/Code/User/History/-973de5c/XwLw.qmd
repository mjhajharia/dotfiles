---
title: "Hierarchical Models - Eight Schools (Centered, Non-Centered)"
author: ""
date: ""
format:
  html:
...
```{julia}
display(funnel_plots(scatter(theta0, logtau, 
                            alpha=0.1, 
                            label=L"\mathrm{Centered: \kappa(H)}", 
                            markerstrokewidth=0, 
                            c="Gray")));
...
display(funnel_plots(scatter(theta0, logtau, 
                            alpha=0.1, 
                            zcolor=log.(CN_Hessian_cholesky), 
                            label=L"\mathrm{Centered: \kappa(H_T)}", 
                            markerstrokewidth=0)));
```
...
```{julia}
Plots.theme(:default)
scatter(CN_Hessian, CN_Hessian_cholesky, xaxis=:log, yaxis=:log, alpha=0.1, label="Centered Eight Schools", font="Palatino Linotype")

display(hessian_transformed_plot(scatter!(CN_Hessian_nc, CN_Hessian_cholesky_nc, xaxis=:log, yaxis=:log, alpha=0.1, label="Non-Centered Eight Schools", font="Palatino Linotype"),"Cholesky"));
```
