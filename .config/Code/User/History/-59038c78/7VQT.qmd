...
includet("snippets/whitening_eightschools.jl")
```
...
```{julia}
display(funnel_plots(scatter(theta0_nc, logtau_nc, alpha=0.1, label=L"\mathrm{Non-Centered: \kappa(H)}", markerstrokewidth=0, c="Gray")));

# CN Hessian
display(funnel_plots(scatter(theta0_nc, logtau_nc, alpha=0.1, zcolor=CN_Hessian_nc, label=L"\mathrm{Non-Centered: \kappa(H)}", markerstrokewidth=0)));

# CN Cholesky Hessian
display(funnel_plots(scatter(theta0_nc, logtau_nc, alpha=0.1, zcolor=CN_Hessian_cholesky_nc, label=L"\mathrm{Non-Centered: \kappa(H_T)}", markerstrokewidth=0)));
```
...
plot1 = scatter(hessian_df[:, :CN_Hessian], hessian_df_cholesky[:, :CN_Transformed_Hessian], xaxis=:log, yaxis=:log, alpha=0.1, label="Centered Eight Schools", font="Palatino Linotype")

plot2 = scatter!(hessian_df_nc[:, :CN_Hessian], hessian_df_cholesky_nc[:, :CN_Transformed_Hessian], xaxis=:log, yaxis=:log, alpha=0.1, label="Non-Centered Eight Schools", font="Palatino Linotype")
