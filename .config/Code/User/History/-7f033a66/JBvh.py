...
def sample(
    ...
    show_progress=True,
    resample=False,
    fstr = lambda *, x: f"{complete_output_dir}/{x}_{pkey}_{n_repeat}.nc",
    inits=None
):
    """
    ...
    idata.to_netcdf(f'{complete_output_dir}/samples_{pkey}_{n_repeat}.nc')

    with open(f'{complete_output_dir}/time_{pkey}_{n_repeat}.txt', 'w') as f:
        f.write(str(time.time() - start_time))

    az.summary(idata).to_csv(f'{complete_output_dir}/summary_{pkey}_{n_repeat}.csv')

    idata.sample_stats.n_steps.to_pandas().to_csv(f'{complete_output_dir}/leapfrog_{pkey}_{n_repeat}.csv')

    np.savetxt(f'{complete_output_dir}/ess_{pkey}_{n_repeat}.csv',np.asarray(ess),delimiter =", ")
