...
def sample(
    ...
    complete_output_dir=f'{output_dir}/samples/{transform}/{evaluating_model}'
    Path(f'{complete_output_dir}').mkdir(parents=True, exist_ok=True)

    idata.to_netcdf(f'{complete_output_dir}/{pkey}_{n_repeat}.nc')

    with open(f'{output_dir}/sample_stats/time_{transform}/{pkey}_{n_repeat}.txt', 'w') as f:
        f.write(str(time.time() - start_time))

    az.summary(idata).to_csv(f'{output_dir}/sample_stats/summary_{transform}_{pkey}_{n_repeat}.csv')
