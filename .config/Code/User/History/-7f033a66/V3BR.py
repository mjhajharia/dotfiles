...
def sample(
    ...
    if resample == False:
        for params in parameters:
            filename = f'{output_dir}sampling_results/{transform}/{evaluating_model}/{param_map[tuple(list(params.values())[0])]}_{n_repeat}.nc'
        ...
        for params in tqdm(parameters):
            ...
            az.summary(idata).to_csv(f'{output_dir}/{transform}_{param_map[tuple(list(params.values())[0])]}_{n_repeat}.csv')
            np.savetxt(f'ess_{transform}_{param_map[tuple(list(params.values())[0])]}_{n_repeat}.csv',np.asarray(ess),delimiter =", ")
