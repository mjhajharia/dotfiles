...
def sample(
    ...
    idata.to_netcdf(f'{complete_output_dir}{param_map[tuple(list(parameters.values())[0])]}_{n_repeat}.nc')
    with open(f'{complete_output_dir}time_{param_map[tuple(list(parameters.values())[0])]}_{n_repeat}.txt', 'w') as f:
        f.write(str(time.time() - start_time))

    az.summary(idata).to_csv(f'{output_dir}/summary/{transform}_{param_map[tuple(list(parameters.values())[0])]}_{n_repeat}.csv')

    idata.sample_stats.n_steps.to_pandas().to_csv(f'{output_dir}/sample_stats/leapfrog_{transform}_{param_map[tuple(list(parameters.values())[0])]}_{n_repeat}.csv')

    np.savetxt(f'{output_dir}/sample_stats/ess_{transform}_{param_map[tuple(list(parameters.values())[0])]}_{n_repeat}.csv',np.asarray(ess),delimiter =", ")
