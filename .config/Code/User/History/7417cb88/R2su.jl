...
function preprocessing_pdb_draws(model_name, posterior_name)
    ...
    if model_name=="eight_schools_centered"
        println("executed")
        mu, tau = constrained_draws[:, end-1], constrained_draws[:, end]
        for i in 1:8
            constrained_draws[:, i] = (constrained_draws[:, i] - mu) ./ tau
        end
    end
    unc_sample_array = vcat([param_unconstrain(bs_model, collect(row))' for row in eachrow(constrained_draws)]...);
...
function get_hessian_script(model_name, posterior_name)
    unc_sample_array, bs_model = preprocessing_pdb_draws(model_name, posterior_name)
    ...
    CSV.write(fullpath(23), Tables.table(unc_sample_array));
