...
function preprocessing_pdb_draws(model_name, posterior_name, unconstrain=true)
    pdb = PosteriorDB.database()
    model = PosteriorDB.model(pdb, model_name)
...
function get_hessian_script(model_name, posterior_name, constrained_draws=false)
    x, bs_model = preprocessing_pdb_draws(model_name, posterior_name, true);

    if constrained_draws != false
        unc_sample_array = vcat([
            param_unconstrain(bs_model, collect(row))' for row in eachrow(constrained_draws)
        ]...);
    else 
...
function retrieve_data(posterior_name)
    ...
    return unc_sample_array, hessian_df_cholesky
end;
